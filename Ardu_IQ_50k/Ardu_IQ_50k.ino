...
void loop() {
  ...
  SPI.transfer (0x63); //LSB [7...0] 63 for I and 65 for Q
  //READ DATA 0X20
  ...
  word dataResultI = MSB << 8;
  dataResultI |=LSB;
  //ADC_CONTROL_REGISTER2 0x10
  SPI.transfer (0x10); //ADCREF connected to VREF, for impedance measurement
  SPI.transfer (0x00); //MSB [15...8] Output of the body composition measurement front-end (OUTP_FILT/OUTM_FILT)
  SPI.transfer (0x65); //LSB [7...0] 63 for I and 65 for Q
  //READ DATA 0X20
  SPI.transfer (0x20); //bit 21 to '1' to read data
  //delay(10);
  //pinMode(MISO, INPUT);
  ...
  Serial.print("50kHz");
  Serial.print("I= ");
  Serial.print(dataResultI);
  Serial.print("; Q= ");
  Serial.println(dataResultQ);
delay(1000);
}

// AFE4300 inicialization, FWR everything set for body composition measurement (BCM)
word initAFE4300(){
digitalWrite (cs, LOW);
